        ...
        if pagesession.login != 1:
            ...
            if dataget.subsys:
                ...
                if dataget.func and dataget.subsys:
                    return subC.mainController(dataget.subsys, dataget.func, db)
                else:
...
class analysisPOST:
    def POST(self):
        ...
        if x.apost == "UBSVM":
            if 'myfile' in x: # to check if the file-object is created
                db.insert('CSVFile', filename=DBfilename, type=filetype, subName='UBSVM')
                raise web.seeother('/analysis?subsys=UBSVM')
        elif x.apost == "CloudKNN":
            if 'myfile' in x: # to check if the file-object is created
                db.insert('CSVFile', filename=DBfilename, type=filetype, subName='CloudKNN')
                raise web.seeother('/analysis?subsys=CloudKNN')
